...
class TextDataset_Pascal(data.Dataset):
    def __init__(self,
                 data_dir,
                 split='train',
                 base_size=64,
...
class TextDataset_Pascal_all(data.Dataset):
    def __init__(self,
                 data_dir,
                 split='all',
                 base_size=64,
    ...
    def build_dictionary(self, all_captions):
        word_counts = defaultdict(float)
        ...
        return [
            train_captions_new, ixtoword, wordtoix,
            len(ixtoword)
        ]

    def load_text_data(self, data_dir, split):
        filepath = os.path.join(data_dir, 'captions_all.pickle')
        all_names = self.load_filenames(data_dir, split)
        if not os.path.isfile(filepath):
            all_captions = self.load_captions(data_dir, all_names)

            all_captions, ixtoword, wordtoix, n_words = \
                self.build_dictionary(all_captions)
            with open(filepath, 'wb') as f:
                pickle.dump(
                    [all_captions, ixtoword, wordtoix],
                    f,
                    protocol=2)
                print('Save to: ', filepath)
        else:
